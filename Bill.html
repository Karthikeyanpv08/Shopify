...
try
{
	...
	ResultSet rs=st.executeQuery("select sum(total)from cart wher email='"+email+"' and status='bill'");
	while(rs.next())
	{
		total=rs.getInt(1);
	}
	...
	{
%>
<h3>Online shopping Bill (Shopify)</h3>
<hr>
...
<h3>Product Details</h3>
  ...
  <% 
  ResultSet rs1=st.executeQuery("select * from cart inner jion product where cart.product_id=product.id and cart.email='"+email+"' and cart.status='bill'");
  while(rs1.next())
  {
	  sno=sno+1;
  %>
